...
# Fallback products shown when the DB table is empty (negative IDs so they
# never collide with real product IDs). These are display-only placeholders;
# add real products via /mk-dashboard/add to replace them.
HARDCODED_PRODUCTS = [
...
def get_products(query='', category=''):
    ...
    # Fallback: only when the table is truly empty and no search/filter was
    # applied, so real "no results found" states still show correctly.
